...
\vspace*{-0.8cm}
When studying manifolds is often proves useful to examine simple structures that live on them. For example, differential forms give rise to the de Rham cohomology, which tells us something about the global structure of the manifold. In this thesis, we study some of the most simple maps: maps from the manifold $M$ to $\R$. A prototypical example of such a map is a projection.
...
\begin{figure*}[H]
    ...
    \sidecaption[][-8ex]{Some examples of cobordisms. 
        When the height function does not have critical points, the topology of the bottom is the same as that of the top.
...
\begin{proof}
    ...
    Because  $f$ has no critical points, the gradient vector field $W = \grad f := (df)^{\sharp}$ never vanishes.\sidenote{Here and hereafter, we use $\sharp$ and $\flat$ for the `musical isomorphisms' induced by a metric $\left< \cdot, \cdot \right>$, given by
        \begin{align*}
...
\end{remark}
\sidenotetext[][-2.0cm]{\fullcite[(i)]{hcobord}}
...
Level sets of values slightly above an below $0$ look locally like hyperbolas on our figure, but remembering that that the axes consist of multiple dimensions, it would be more precise to call them hyperboloids. We give some more concrete examples for $n=2,3$ and  $k=0,1,2,3$ in the following table:
\begin{table}[H]
    \centering
    \sidecaption{Some examples of Morse charts in dimension two and three.\label{fig:more-concrete-morse-charts}}
...
To better understand the behaviour of the critical point,
...
It is bounded by level sets $f(p) = \pm \epsilon$ and negative\sidenote{
While it does not really matter if we choose the negative or positive gradient, it is customary in Morse theory to use the latter.
The gradient flow of a height function flows then from top to bottom which is arguably more intuitive when one thinks of it like water flowing down a mountain.
} gradient flow lines of  $f$.
where gradient is to be interpreted with respect to the standard metric on $\R^{n}$.\sidenote{More explicitly, if $f$ is Morse, we know that locally around a critical point $p$, $f$ is given by
    \begin{align*}
...
\end{marginfigure}
In Figure~\ref{fig:morse-chart-zoomed-in}, the cut out area is coloured in yellow and indicated with \circled{H}.
...
\begin{definition}[$k$-handle]
    ...
    The bottom of the handle, $S_k = \sphere{k-1} \times \cdisk{n-k}$ is called the \emph{attachment region}.
    \end{definition}
\end{itemize}
    ...
    In general, the act of attaching a $k$-handle to a manifold $N$ is glueing a handle $\cdisk{k} \times \cdisk{n-k}$ to the boundary of $N$ along its attachment region $S_k$ via a chosen embedding $S_k \to  \partial N$.
    We should remark that the resulting manifold is not always smooth, although there exists techniques to `smooth the corners', but we will not go deeper into these subtleties.
    ...
    Proofs of these statements can be found in Morse Theory and Floer Homology by Audin and Damian\sidecite[32]{audin}.
...
\begin{itemize}
    \item \smallstrutt 0-handle $\cdisk0 \times \cdisk{1} = [-1, 1]$, with attachment region $\O$. \hfill \minifig{1cm}{handle-1-0} 
        
    \item \smallstrutt 1-handle   $\cdisk{1} \times  \cdisk{0} = [-1, 1]$ with attachment region $S^{0} = \{-1, 1\}$.  \hfill \minifig{1cm}{handle-1-1}
\end{itemize}
...
\begin{itemize}
    \item \smallstrutt 0-handle  $\cdisk{0} \times \cdisk{2}$ with attachment region $\O$. \hfill \minifig{1cm}{handle-2-0}
    \item \smallstrutt 1-handle  $\cdisk{1} \times \cdisk{1}$ with attachment region $\cdisk1 \times S^{0}$. \hfill\minifig{1cm}{handle-2-1}
    \item \smallstrutt 2-handle  $\cdisk{2} \times \cdisk{0}$ with attachment region $\cdisk{0} \times S^1$. \hfill\minifig{1cm}{handle-2-2}
    \end{itemize}
\begin{marginfigure}
    \centering
...
\begin{itemize}
    \item  \smallstrutt 0-handle  $ \cdisk0 \times \cdisk{3}$ with attachment region $\O$ 
        \hfill\minifig{1cm}{handle-3-0}
    \item \smallstrutt 1-handle  $ \cdisk1 \times \cdisk{2}$ with attachment region $ \cdisk2 \times S^{0}$
        \hfill\minifig{1cm}{handle-3-1}
    \item \smallstrutt 2-handle  $ \cdisk2 \times \cdisk{1}$ with attachment region $ \cdisk1 \times S^1$
        \hfill\minifig{1cm}{handle-3-2}
    \item \smallstrutt 3-handle  $ \cdisk3 \times \cdisk{0}$ with attachment region $S^{2}$
...
\begin{eg}[Handle decomposition of $S^{3}$]
    ...
    Then handle decomposition is then a $0$-handle ($\cdisk{3}$) glued to a $3$-handle (which is also diffeomorphic to $\cdisk{3}$).
\end{eg}
...
\begin{eg}[$T^{3} = S^{1} \times S^{1} \times S^{2}$]
    ...
    As $\cos(2 \pi \frac{1}{2}) < 0$ and $\cos(2 \pi 0) > 1$, we find that the index of a critical point $(x, y, z)$ is the number of zeros occurring in the triplet $(x, y, z)$. Hence we get the following table of critical points:
    \[\begin{array}{cccccc}
...
\begin{eg}
    ...
    Here it is also easy to see that after an isotopy, the $1$- en $2$-handle can be cancelled.

    This is also an example where the order of the handles is important: we cannot attach the $2$-handle before the $1$-handle, in contrary to for example the handle decomposition of $ S^1 \times S^2$ given earlier.
...
In this section we show that any manifold admits uncountably many Morse functions and that the set of Morse function form a dense subset of all smooth maps $M \to  \R$.
...
We choose an embedding of $M$ where $f$ is the first coordinate on $M$, so we can think of $f$ as a simple projection: $x \mapsto x_1$. \todo{Why is this possible?}

...
\begin{figure*}[H]
    ...
    \sidecaption[][-8ex]{Some examples of cobordisms. 
        When the height function has a critical point, the topology can change from top to bottom.
...
\todo{Not in the case of the higher genus example \ldots Change wording!}
...
\begin{definition}
    ...
    We call $S_k = \sphere{k-1} \times \cdisk{n-k}$ the attachment region.

    The act of attaching a $k$-handle to a manifold $M$ is glueing a handle $\cdisk{k} \times \cdisk{n-k}$ to the boundary of $M$ along its attachment region $S_k$ via a chosen embedding $S_k \to  \partial M$.
\end{definition}
\begin{remark}
...
\begin{proof}
    ...
    Splitting $M$ between critical values,\todo{Better wording} we obtain a set of cobordisms with a single critical point.
...
Before discussing some examples, let us first discuss some of the different ways to look at handles.
...
Handles in dimension one are the simplest ones. We only have two types:

\begin{description}
    \item[0-handle] $\cdisk0 \times \cdisk{1} = [-1, 1]$, with attachment region $\O$. \hfill \minifig{1cm}{handle-1-0} 
        
    \item[1-handle]   $\cdisk{1} \times  \cdisk{0} = [-1, 1]$ with attachment region $S^{0} = \{-1, 1\}$.  \hfill \minifig{1cm}{handle-1-1}
\end{description}
...
However, we can construct an isotopy from the second to the first embedding that cancels two critical points.
Interestingly, the height function is Morse for all $t$, except at the exact moment when the cancellation happens, indicated in yellow in the below.
...
\begin{description}
    \item[0-handle] $\cdisk{0} \times \cdisk{2}$ with attachment region $\O$. \hfill \minifig{1cm}{handle-2-0}
    \item[1-handle] $\cdisk{1} \times \cdisk{1}$ with attachment region $\cdisk1 \times S^{0}$. \hfill\minifig{1cm}{handle-2-1}
    \item[2-handle] $\cdisk{2} \times \cdisk{0}$ with attachment region $\cdisk{0} \times S^1$. \hfill\minifig{1cm}{handle-2-2}
\end{description}
...
\begin{description}
    ...
    \item[1-handle] $ \cdisk1 \times \cdisk{2}$ with attachment region $ \cdisk2 \times S^{0}$
        \hfill\minifig{1cm}{handle-3-1}
    \item[2-handle] $ \cdisk2 \times \cdisk{1}$ with attachment region $ \cdisk1 \times S^1$
        \hfill\minifig{1cm}{handle-3-2}
    \item[3-handle] $ \cdisk3 \times \cdisk{0}$ with attachment region $S^{2}$
...
Let us first recall the following result, due to Whitney\sidecite{todo}.

...
\vspace*{-0.8cm}
When studying manifolds it often proves useful to examine simple structures that live on them. For example, differential forms give rise to the de Rham cohomology, which tells us something about the global structure of the manifold. In this thesis, we study some of the most simple maps: maps from the manifold $M$ to $\R$\sidenote[1]{This superscript is also a footnote. In this thesis, we will only consider maps that are $C^{\infty}$.}. A prototypical example of such a map is a projection.
...
\begin{table}[H]
    \centering
    \sidecaption[][-0.5cm]{Some examples of Morse charts in dimension two and three.\label{fig:more-concrete-morse-charts}}
...
\begin{itemize}
    ...
    \item \smallstrutt 1-handle  $ \cdisk1 \times \cdisk{2}$ with attachment region $ S^{0} \times \cdisk 2$
        \hfill\minifig{1cm}{handle-3-1}
    \item \smallstrutt 2-handle  $ \cdisk2 \times \cdisk{1}$ with attachment region $ S^{1} \times \cdisk1$
        \hfill\minifig{1cm}{handle-3-2}
    \item \smallstrutt 3-handle  $ \cdisk3 \times \cdisk{0}$ with attachment region $S^{2} \times B^{0}$
...
We choose an embedding of $M$ where $f$ is the first coordinate on $M$, so we can think of $f$ as a simple projection: $x \mapsto x_1$.\sidenote{
    Start with an embedding $\phi: M \hookrightarrow \R^{n}$ for large enough $n$.
    Then consider  $\psi: M \to  \R^{n+1}: x\mapsto (f(x), \Phi(x))$.
    Then the height function of this embedding corresponds is exactly $f$.
}

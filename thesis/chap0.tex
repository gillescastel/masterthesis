...
\begin{prop}
    Let $M$ be a manifold and $N_1, N_2$ be two submanifolds. If $ N_1 \tcap N_2$, then 
    \[
        \codim (N_1 \tcap N_2) = \codim N_1 + \codim N_2
    ,\] 
    where the codimensions with respect to $M$, i.e. $\codim N_i = \dim M - \dim N_i$.\label{prop:transverse-codimensions-add}
\end{prop}
\begin{proof}

...
\begin{theorem}[Smale's theorem]
    If $M$ is a simply connected closed manifold of dimension  $m \ge  6$ and $H_\bul(M)$ is free, then there exists a Morse function on  $M$ such that  $\# \Crit_k f = r_0(H(M, \Z))$ for all $k = 0, \ldots, n$.
\end{theorem}
...
\section{Cancellation of points in the middle dimensions}
...
Moreover, the second cancellation theorem allows remove columns and rows of $[\partial_k]$ under certain conditions.
...
In this section, we will show that we can eliminate $1$-handles by replacing them with $3$-handles.

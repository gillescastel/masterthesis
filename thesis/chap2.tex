...
In this chapter, we want to address two natural questions that came up when discussing examples of handle decompositions: `When can we reorder handles in a handle decomposition?' and `Under what conditions can we cancel two critical points via an isotopy?'
...
\end{eg}
    ...
    An obtrusion to attaching two handles at the same time would then that they intersect before reaching the bottom. We will make this more precise later, but it gives an idea as for why we might be interested in intersections of (un)stable manifolds.
    ...
    \begin{definition}[Smale condition]
        A pseudo-gradient field addapted to a Morse function $f$ is said to satisfy the \emph{Smale condition} if all for all pairs of critical points $ \{p, q\}  \subset \Crit f$, we have that $\stable{p}$ intersects  $\unstable{q}$ transversally, i.e. 
        \[
    ...
    \begin{noneg}
        ...
        Even more, if the condition would be satisfied, there should be no trajectories connecting $b$ and $c$, because both have index $1$, so $\dim (\stable{b} \tcap \unstable{c}) = 0$. However, as illustrated in the figure, we have two.
    \end{noneg}
    \begin{eg}
        ...
        The intersection of $\stable{b}$ and  $\unstable{c}$ is not tangent any more: indeed, they do not even intersect at all. We have illustrated below in Figure~\ref{fig:torus-tilted-height-function-stable-and-unstable-manifolds}.
    \end{eg}
    \begin{figure}[H]
        \centering
        \sidecaption{
            When embedding the torus in $\R^3$ tilted, the Smale condition is satisfied: all stable and unstable manifolds intersect transversally.
            Indeed, stable and unstable manifolds of $c$ and $b$ don't intersect at all.
...
Now that we have introduced the (un)stable manifolds, we are ready to prove a rist reordering theorem. The statement and proof can be found in notes of `Lectures on the $h$-Cobordism Theorem' by Milnor\sidecite{hcobord}.
\begin{theorem}
...
\begin{theorem}[Francois Laudenbach\sidenotemark]
    Let $f: M \to  \R$ be a Morse function and $X$ be pseudo-gradient field.
...
More precisely, there exists an isotopy of functions $f_t$ starting at $f_0=f$ supported in an open neighborhood of the closure of  $\unstable{p} \cap  \{f \ge  f(q) - \epsilon\} $ such that
\begin{itemize}
